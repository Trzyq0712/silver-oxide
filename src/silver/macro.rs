...
impl<'i> AstWalkerMut<'_> for MacroInliner<'i> {
    ...
    fn walk_mut_statement(&mut self, stmt: &mut Statement) {
            ...
            && call.kind == Some(StmtCallKind::Macro)
        {
            ...
            let mut expanded_stmts = match &macro_def.body {
            ...
            for statement in &mut expanded_stmts.0 {
                statement.walk_mut(&mut substitutor);
                statement.walk_mut(self);
            }
            self.exit_expansion();

            *stmt = Statement::Block(expanded_stmts);
        }
    }
}
