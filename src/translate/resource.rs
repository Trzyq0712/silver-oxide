...
impl<'sil> ResourceBuilder<'sil> {
    pub fn new_for_requires(signature: &silver::Signature) -> Self {
        todo!()
    }

    pub fn new_for_ensures(signature: &silver::Signature, precond: Option<vmir::MemberId>) -> Self {
        todo!()
    }
    ...
    pub fn build(mut self, exp: &silver::HeapExp) -> Result<vmir::Resource, ()> {
        ...
        let type_table = self.tc.type_check().unwrap();

        todo!("Typecheck + build the resource")
    }
}
